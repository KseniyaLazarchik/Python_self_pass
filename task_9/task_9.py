...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            file_reader = JsonReader(file_path)
            file_reader.process_json()

        elif choice == '6':
            file_path = input(f"Enter the xml file path (default: {DEFAULT_INPUT_XML}): ").strip() or DEFAULT_INPUT_XML
            file_reader = XMLReader(file_path)
            file_reader.process_xml()
